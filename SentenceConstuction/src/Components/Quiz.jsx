...
function  Quize(){
    const quest={
      ...
      const questionRef = useRef(quest.data.questions[0]);
      const prevquestionRef=useRef(null);
   
   function checkans(){
    let c=0;
    for(let i=0;i<4;i++){
        console.log(`ans[i]=${ans[i]} and ${prevquestionRef.current.options[i]}`);
        
        if(ans[i]==prevquestionRef.current.options[i]){
            console.log("correct");
            c++;
  ...
  useEffect(() => {
    ...
    const Timer=setTimeout(() => {
      ...
      console.log(questionRef.current.options);
        
      if(count===0){
        console.log(index);
        
        setIndex((index)=> index+1);
        prevquestionRef.current=questionRef.current;
        checkans()
        const newIndex=index+1;
        console.log(newIndex);
        
        if(newIndex>=quest.data.questions.length){
            setFlag(1);
            return;
        }
        questionRef.current=quest.data.questions[newIndex];
        setIndex(newIndex);
  ...
  return (
    <div>
      ...
      {flag === 0 ? (
        <div>
          <h1>{count}</h1>
          <p>{questionRef.current.question}</p>
          {questionRef.current.options.map((opt, i) => (
            <button key={i} onClick={() => pushans(opt,i)}>{opt}</button>
          ))}
        </div>
      ) : (
        <h1>Total{total}</h1>
